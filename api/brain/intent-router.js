...
function nameHasSize(name, size) {
  ...
  return n.includes(String(size)) || (
    ...
    size === 32 && /\b(srednia|œrednia|medium)\b/.test(n) ||
...
function baseDishKey(name) {
  ...
  n = n
    ...
    .replace(/\b(duza|du¿a|mala|ma³a|srednia|œrednia|xl|xxl|small|medium|large)\b/g, ' ')
    .replace(/\(.*?\)/g, ' ')
    .replace(/\s+/g, ' ')
    .trim();
...
function dedupHitsByBase(hits, preferredSize = null) {
  ...
  for (const [, arr] of groups) {
    ...
    // auto-pick po rozmiarze, jeœli podano w tekœcie
    if (preferredSize) {
...
const NAME_ALIASES = {
  ...
  'margerita': 'pizza margherita',  // czêsty b³¹d STT
  'margarita': 'pizza margherita',  // czêsty b³¹d STT
  'pepperoni': 'pizza pepperoni',
  'hawajska': 'pizza hawajska',
  'hawajskiej': 'pizza hawajska',
  'diavola': 'pizza diavola',
  'diabolo': 'pizza diavola',       // czêsty b³¹d STT/pronunciation
  'diabola': 'pizza diavola',       // czêsty b³¹d STT/pronunciation
  'pizza diabolo': 'pizza diavola', // pe³na nazwa z b³êdem
  ...
  'rolada': 'rolada œl¹ska',
  'rolade': 'rolada œl¹ska',
  'rolady': 'rolada œl¹ska',

  // Pierogi
  'pierogi': 'pierogi z miêsem',
  'pierogów': 'pierogi z miêsem',
  'pierogami': 'pierogi z miêsem',

  // W³oskie
  'lasagne': 'lasagne bolognese',
  'lasania': 'lasagne bolognese',  // czêsty b³¹d STT
  ...
  'pad taj': 'pad thai z krewetkami',  // czêsty b³¹d STT
  'padthai': 'pad thai z krewetkami',
  'sajgonki': 'sajgonki z miêsem',
  'sajgonek': 'sajgonki z miêsem',
  'sajgonkami': 'sajgonki z miêsem',
  ...
  // Specjalny wyj¹tek: G³odzilla (Klaps Burgers) — ³ap tak¿e przekrêcenia „godzilla”
  'g³odzilla': 'g³odzilla',
  'glodzilla': 'g³odzilla',
  'godzilla': 'g³odzilla',
  // krótsze rdzenie, aby z³apaæ odmiany (np. „g³odzillê”, „godzilli”, „glodzille”)
  'g³odzil': 'g³odzilla',
  'glodzil': 'g³odzilla',
  'godzil': 'g³odzilla',
};
 ...
 * Jeœli alias nie znaleziony › zwraca unknown_item, nie failuje
 */
const DETERMINISTIC_ALIAS_MAP = {
  ...
  // Pizza (zachowane z NAME_ALIASES dla kompatybilnoœci)
  'margherita': 'pizza margherita',
  'margherite': 'pizza margherita',
  'margheritê': 'pizza margherita',
 ...
 * 
 * ZMIANA ZACHOWANIA:
 * - U¿ywa deterministycznej mapy aliasów (nie fuzzy-match)
 * - Jeœli alias nie znaleziony › zwraca oryginalny tekst (nie failuje)
...
export function applyAliases(text) {
  ...
  // Przeszukaj deterministyczn¹ mapê aliasów
  ...
  // Zawsze zwróæ string (nawet jeœli brak zamian)
...
async function loadMenuCatalog(session) {
  // preferuj ostatni¹ restauracjê z kontekstu, jeœli jest
...
function extractRequestedItems(text) {
  // Wyodrêbnij ¿¹dane pozycje z tekstu (proste rozpoznawanie po aliasach i nazwach)
...
// Rozpoznaj wiele dañ w jednym tekœcie (split by "i", "oraz", ",")
function splitMultipleItems(text) {
  // Usuñ s³owa kluczowe zamówienia
  let cleaned = text
    .replace(/\b(zamów|zamówiæ|poproszê|chcê|wezmê|chcia³bym|chcia³abym)\b/gi, '')
    .trim();

  // Split by separators
  const parts = cleaned.split(/\s+(i|oraz|,)\s+/i).filter(p => p && !['i', 'oraz', ','].includes(p.toLowerCase()));

  // Jeœli nie ma separatorów, zwróæ ca³y tekst
  if (parts.length <= 1) {
    return [text];
  }
...
export function parseOrderItems(text, catalog) {
  ...
  // 1. Strip courtesy/order prefixes EARLY to prevent "Poproszê X" becoming an unknown item
  const PREFIXES = /^(poproszê|zamawiam|wezmê|dodaj|chcia³bym|chcia³abym|proszê|biorê|dla\s+mnie)\s+/i;
  ...
  try {
    ...
    // Jeœli applyAliases zwróci³o orygina³ i nie znalaz³o aliasu,
    // sprawdŸ czy to mo¿e byæ unknown_item
    if (textAliased === cleanText) {
      // SprawdŸ czy tekst nie pasuje do ¿adnego aliasu
      ...
      if (!hasKnownAlias && cleanText.trim().length > 0) {
        // Mo¿e byæ unknown_item - zapisz do póŸniejszej weryfikacji
  ...
  if (!catalog || !Array.isArray(catalog) || catalog.length === 0) {
    ...
    return {
      ...
      needsClarification: true, // Wymaga wyjaœnienia (brak katalogu)
      missingAll: true,
  ...
  for (const itemText of itemTexts) {
    ...
    try {
      const qty = extractQuantity(itemText) || 1; // Domyœlnie 1 jeœli brak iloœci
      const hits = catalog
        .filter(it => {
          try {
            ...
            return false; // Bezpieczne - nie dopasuj jeœli b³¹d
          }
        })
        .map(it => ({
      ...
      // Kontynuuj z nastêpnym itemem - nie failuj ca³ego parsowania
  ...
  try {
    ...
    selected = allHits; // Fallback - u¿yj wszystkich hitów
    clarifications = [];
  }

  // SprawdŸ czy s¹ niedostêpne pozycje (fallback) – nie psuj g³ównego dopasowania
  ...
  const fuzzyNameHit = (needle, haystackName) => {
    try {
      ...
      // l¿ejszy próg: przynajmniej 1 wspólny token >2 znaków
  ...
  // Pozycjê uznajemy za „dostêpn¹”, jeœli:
  // - jest w matched (availableNames) ORAZ fuzzy pasuje, LUB
  // - nie jest w matched (np. wymaga doprecyzowania rozmiaru), ale wystêpuje w ca³ym katalogu (te¿ fuzzy)
  const unavailableNames = requestedNames.filter(requestedName => {
    // 1) SprawdŸ na liœcie ju¿ dopasowanych
    const inMatched = availableNames.some(an => fuzzyNameHit(requestedName, an));
    if (inMatched) return false;

    // 2) SprawdŸ w ca³ym katalogu (by nie oznaczaæ jako unavailable, gdy s¹ warianty wymagaj¹ce clarify)
  ...
  for (const h of matched) {
    try {
      if (!h || !h.restaurant_id) continue; // Pomiñ nieprawid³owe hitów
      ...
      // Kontynuuj z nastêpnym itemem
    }
  }

  // Jeœli s¹ unknown items i nie znaleziono dopasowañ, dodaj je do unavailable
...
/**
 * Bezpieczny fallback - zawsze zwraca jakiœ intent
 */
...
// Helper do wykrywania intencji eksploracyjnej (pytania o menu/ofertê)
function isExploratory(text) {
  ...
  if (/^poka[z¿]/.test(t) && !/\b(zamawiam|bior[êe]|poprosz[êe])\b/.test(t)) return true;
  return false;
}
...
export async function detectIntent(text, session = null, entities = {}) {
  ...
  try {
    ...
    // ==========================================
    // Wykryj intencjê NA PODSTAWIE ZAMIARU, nie frazy
    const functionalIntent = detectFunctionalIntent(text, session);

    // Jeœli wykryto funkcjonalny intent (ADD_ITEM, CONTINUE_ORDER, etc.)
    // i ma wysok¹ pewnoœæ, zwróæ go od razu (bez parsowania treœci)
    ...
    // ==========================================
    // ETAP 2: PARSOWANIE TREŒCI (CO KONKRETNIE)
    // ==========================================
    // Dopiero po wykryciu intentu parsuj produkty, iloœci, warianty
    ...
    // ?? SUPER-EARLY EXIT: Pytania "gdzie zjeœæ …" zawsze traktuj jako find_nearby
    ...
    // ======================================================================
    const GREETING_PATTERNS = /^(cze[sœ][cæ]|hej|hej\s|witaj|dzieñ\s+dobry|dzien\s+dobry|siema|siemanko|yo|hi|hello|dobry\s+wieczór|dobry\s+wieczor|dobranoc|serwus|moro|hejka|elo|czeœæ|czesc)([!.,?\s].*)?$/i;
    ...
    // Logika potwierdzania zamówieñ jest teraz obs³ugiwana przez:
    // 1. boostIntent() w brainRouter.js (wykrywa confirm_order/cancel_order)
    // 2. case "confirm_order" i "cancel_order" w brainRouter.js
    // Ta sekcja zosta³a usuniêta, aby unikn¹æ konfliktów z session.pendingOrder
    ...
    // SprawdŸ czy u¿ytkownik ma ju¿ restauracjê w sesji
    let targetRestaurant = null;
    let restaurantsList = null; // ?? Cache dla póŸniejszego u¿ycia
    ...
    // ?? SprawdŸ czy tekst zawiera silne wskaŸniki nowej restauracji
    ...
    // ?? Uruchom agresywne wykrywanie restauracji TYLKO jeœli:
    // 1. NIE MA restauracji w sesji, LUB
    // 2. Tekst zawiera silne wskaŸniki nowej restauracji
    ...
    if (shouldSearchRestaurants) {
      ...
      try {
        ...
        if (restaurantsList?.length) {
          ...
          // ?? Early exit: sprawdŸ najpierw exact match (szybkie)
          ...
          // ?? Fuzzy match tylko jeœli exact match nie zadzia³a³
          if (!targetRestaurant) {
            ...
            for (const r of restaurantsList) {
              ...
              for (const nameWord of nameWords) {
                // ?? Optymalizacja: sprawdŸ najpierw exact match s³owa (szybkie)
                if (textWords.includes(nameWord)) {
                  matchedWords++;
                  continue;
                }

                // ?? Levenshtein alleen voor woorden >= 7 znaków (krótkie s³owa › exact match)
        ...
        // ?? Nie rzucaj b³êdu - kontynuuj z session restaurant
      }
    } else {
    ...
    try {
      ...
      // ======================================================================
      const ORDER_VERB_GATE = /\b(zamawiam|zamów|zamow|poproszê|proszê|poprosz[êe]|chcê|chce|wezmê|wezm[êe]|biore|bior[êe]|dodaj|dla\s+mnie|chcia³(bym|abym)|skusz[êe]|zdecyduj[êe]|lec[êe]\s+na)\b/i;
      const QUANTITY_GATE = /\b(\d+\s*(x|razy|sztuk)?|dwa|dwie|trzy|cztery|piêæ|jeden|jedna)\b/i;
      ...
      if (!passesOrderGate) {
        ...
        if (parsed.missingAll) {
          ...
          return {
            intent: 'no_menu_items',
            reply: 'Nie znalaz³am ¿adnych pozycji w menu tej restauracji. Mo¿e chcesz sprawdziæ coœ innego?',
            confidence: 0.8,
            fallback: true
          };
        }

        // SprawdŸ czy s¹ niedostêpne pozycje (nawet jeœli parsed.any === false)
        // ?? ALE: jeœli tekst zawiera nazwê restauracji, to nie zwracaj clarify_order
        // (user mo¿e mówiæ np. "klaps burger" = nazwa restauracji, a nie zamówienie)
        if (parsed.unavailable && parsed.unavailable.length > 0 && parsed.needsClarification) {
          // Jeœli parser i tak coœ znalaz³ (available), preferuj create_order zamiast clarify
          ...
          // ?? OPTIMIZATION: U¿yj cache z KROK 1 zamiast robiæ nowy query
            ...
            for (const r of restaurantsList) {
              ...
              for (const nameWord of nameWords) {
                if (textWords.includes(nameWord)) {
                  ...
                  // Levenshtein tylko jeœli exact match nie zadzia³a³
                  for (const textWord of textWords) {
                    const dist = levenshteinHelper(textWord, nameWord);
                    if (dist <= 1) {
                      matchedWords++;
          ...
          // Jeœli tekst NIE zawiera nazwy restauracji, to zwróæ clarify_order
          if (!containsRestaurantName) {
            ...
            return {
              intent: 'clarify_order',
              parsedOrder: parsed,
              reply: `Nie znalaz³am aktualnie ${missing} w menu ${restaurantName}, mo¿e chcia³byœ coœ innego?`,
              confidence: 0.9,
        ...
        if (parsed.any) {
          ...
          if (isRestaurantAmbiguous) {
            ...
            return {
              ...
              reply: `Tê pozycjê serwuje kilka restauracji: ${parsed.groups.map(g => g.restaurant_name).join(', ')}. Z której mam zamówiæ?`,
              confidence: 0.95
            };
          }
    ...
    // Bazowe s³owa kluczowe (BEZ polskich znaków - znormalizowane przez normalizeTxt)
    ...
    const menuKeywords = [
      ...
      'dania', 'potrawy', 'co serwuja', 'co podaja', 'karta dan', 'karta dañ',
      'co jest dostepne', 'co dostepne', 'co maja w menu'
    ];

    const orderKeywords = [
      'zamow', 'poprosze', 'prosze', 'chce zamowic', 'zloz zamowienie', 'zamowic cos',
      'dodaj do zamowienia', 'zloz', 'wybieram', 'biore', 'wezme', 'chce', 'chcê'
    ];
    ...
    // Deduplikacja — usuñ duplikaty miêdzy bazowymi a dynamicznymi
    ...
    // ?? KROK 4: Jeœli w early dish detection znaleziono restauracjê, ale nie znaleziono dañ
    // to zwróæ odpowiedni intent na podstawie s³ów kluczowych
    ...
    // S³owa kluczowe ju¿ zdefiniowane wczeœniej
    ...
    // ?? PRIORYTET 0: SprawdŸ czy w tekœcie jest iloœæ (2x, 3x, "dwa razy", etc.)
    // Jeœli tak, to najprawdopodobniej user chce zamówiæ, nie wybieraæ restauracji
    ...
    // ?? PRIORYTET 1: SprawdŸ czy w tekœcie jest nazwa restauracji (fuzzy matching)
    // ?? WA¯NE: Jeœli session.lastRestaurant istnieje i tekst zawiera s³owa kluczowe zamówienia,
    ...
    const hasPizzaKeyword = /\bpizz/i.test(lower); // pizza/pizze/pizzy/pizzê etc.
    ...
    if (hasLastRestaurant && (hasOrderKeyword || hasPizzaKeyword || hasDishKeyword)) {
      ...
      // Nie szukaj innych restauracji - zwróæ create_order z restauracj¹ z sesji
      return { intent: 'create_order', restaurant: session.lastRestaurant };
    }

    // Jeœli tak, to najprawdopodobniej user chce wybraæ restauracjê lub zobaczyæ menu
    console.log('?? PRIORYTET 1: Sprawdzam restauracje w tekœcie:', text);

    // ?? U¿yj cache z KROK 1 jeœli dostêpny, w przeciwnym razie pobierz
    if (!restaurantsList) {
    ...
    if (restaurantsList?.length) {
      ...
      for (const r of restaurantsList) {
        const normalizedName = normalizeTxt(r.name);
        console.log('?? Sprawdzam restauracjê:', r.name, '->', normalizedName);

        // SprawdŸ czy nazwa restauracji jest w tekœcie (fuzzy match)
        // 1. Exact substring match
        if (normalizedText.includes(normalizedName)) {
          console.log('? Exact match found:', r.name);
          // Jeœli jest "menu" › menu_request
          if (allMenuKeywords.some(k => lower.includes(k))) {
            return { intent: 'menu_request', restaurant: r };
          }
          // Jeœli jest "zamów"/"wybieram" › create_order
        ...
        // 2. Fuzzy match — sprawdŸ czy s³owa z nazwy restauracji s¹ w tekœcie
        const nameWords = normalizedName.split(' ');
        const textWords = normalizedText.split(' ');
        let matchedWords = 0;
        ...
        for (const nameWord of nameWords) {
          ...
          // ?? Fuzzy tylko dla s³ów >= 7 znaków (zapobiega "testy"›"tasty")
          if (nameWord.length >= 7) {
            for (const textWord of textWords) {
        ...
        // Jeœli ?75% s³ów z nazwy restauracji pasuje › uznaj za match
        if (matchedWords >= threshold) {
    ...
    // ?? PRIORYTET 2: SprawdŸ menu keywords (bardziej specyficzne ni¿ order)
    if (allMenuKeywords.some(k => lower.includes(k))) {
      updateDebugSession({
        intent: 'menu_request',
        restaurant: null,
    ...
    // ?? PRIORYTET 3: SprawdŸ order keywords
    if (allOrderKeywords.some(k => lower.includes(k))) {
      updateDebugSession({
        intent: 'create_order',
        restaurant: null,
    ...
    // ?? PRIORYTET 4: SprawdŸ nearby keywords
    ...
    // Jeœli Amber nie zna frazy — zapisuje j¹ do bazy do przysz³ego uczenia
    try {
    ...
    // Bezpieczny fallback - zawsze zwróæ jakiœ intent (NIE 'none')
    ...
    // Bezpieczny fallback - zawsze zwróæ jakiœ intent (NIE throw, NIE crash)
  ...
  try {
    switch (intent) {
      case "select_restaurant": {
        ...
        return { reply: "Restauracja wybrana, przechodzê do brainRouter..." };
      }

      case "create_order": {
        const restaurant = session?.lastRestaurant;
        if (!restaurant) {
          return { reply: "Najpierw wybierz restauracjê, zanim z³o¿ysz zamówienie." };
        }
        ...
        try {
          ...
          return { reply: "Nie uda³o siê utworzyæ zamówienia. Spróbuj ponownie." };
        }
      }
      ...
      case "menu_request": {
        const restaurant = session?.lastRestaurant;
        if (!restaurant) {
          return { reply: "Najpierw wybierz restauracjê, ¿ebym mog³a pobraæ menu." };
        }
        ...
        try {
          ...
          if (error) {
            ...
            return {
              ...
              intent: "menu_request",
              restaurant,
              reply: "Nie mogê pobraæ danych z bazy. SprawdŸ po³¹czenie z serwerem.",
            };
          }
          ...
          return {
            reply: `W ${restaurant.name} dostêpne: ${menu
              .map((m) => `${m.name} (${Number(m.price_pln).toFixed(2)} z³)`)
              .join(", ")}.`,
          };
        } catch (err) {
          console.error("?? menu_request error:", err.message);
          return { reply: "Nie mogê pobraæ menu. SprawdŸ po³¹czenie z baz¹." };
        }
      }
      ...
      case "find_nearby": {
        try {
          ...
          return {
            reply:
              "W pobli¿u mo¿esz zjeœæ w: " +
              data.map((r) => `${r.name} (${r.city || r.address})`).join(", "),
          };
        } catch (err) {
          console.error("?? find_nearby error:", err.message);
          return { reply: "Nie mogê pobraæ listy restauracji. SprawdŸ po³¹czenie." };
        }
      }

      case "none":
        return { reply: "Nie jestem pewna, co masz na myœli — spróbuj inaczej." };
    ...
    return { reply: "Wyst¹pi³ b³¹d podczas przetwarzania. Spróbuj ponownie." };
  }
}
